...
const { calculateScoresByUsers } = require('../../client/src/homepage/pro-play-dashboard/utils/pro-play-dashboard.util');
...
const writePredictionsToFile = () => {
    db.collection('predictions').get().then(snapshot => {
        ...
        try {
            fs.writeFileSync('./old-data/spring-2021-predictions.json', JSON.stringify(predictions));
        } catch (err) {
            console.error(err);
        }
    });
...
const getEndOfSplitAnalytics = async () => {
    ...
    for (let prediction of allPredictions) {
        const { userId, id } = prediction;
        if (!mappedPredictions[userId]) {
            mappedPredictions[userId] = {};
        }
        mappedPredictions[userId][id] = prediction;
    }
